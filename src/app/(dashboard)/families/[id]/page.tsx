...
// Fetches the family + its active members and overlays the decrypted view
// fields. Returns null for "doesn't exist" and "archived" alike — archived
// families live only at /families/archived (ADMIN); blocking direct-URL
// access here keeps archived PII from loading for any canViewPeople role.
async function getFamilyForDisplay(id: number) {
...
type DisplayFamily = NonNullable<Awaited<ReturnType<typeof getFamilyForDisplay>>>["displayFamily"]

// Read-only: no giving access returns the empty shape so the caller never
// needs its own "can I see this" branch.
async function getFamilyGiving(familyId: number, canView: boolean) {
  const transactions = canView
    ? (await prisma.transaction.findMany({
        where: { familyId, isGiving: true },
        orderBy: { date: "desc" },
  ...
  const byYear = transactions.reduce<Record<number, typeof transactions>>((acc, t) => {
    ...
    <div className="flex flex-wrap gap-2 print:hidden">
      ...
      {userCanEdit && (
        <Button variant="outline" size="sm" className="h-11 sm:h-7 any-pointer-coarse:h-11" asChild>
          <Link href={`/families/${familyId}/edit`}>Edit</Link>
        </Button>
      )}
      {userCanEdit && (
        <InviteFamilyButton familyId={familyId} defaultEmail={primaryEmail} />
      )}
      {userIsAdmin && (
        <ArchiveFamilyButton familyId={familyId} memberCount={memberCount} />
      )}
    </div>
  )
}
...
function PendingUpdateBanner({ submissionId }: Readonly<{ submissionId: number }>) {
  return (
    <div className="rounded-md bg-warning/10 border border-warning/40 px-4 py-3 text-sm text-warning print:hidden">
      ...
      <Link href={`/families/updates/${submissionId}`} className="underline font-medium">Review now</Link>
    </div>
  )
}
    ...
    <Card>
      ...
      <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p>{[family.address, family.suburb, family.state, family.postcode].filter(Boolean).join(", ") || "—"}</p>
        </div>
        <div>
          <span className="text-muted-foreground">Phone</span>
          <p>{family.homePhone ?? "—"}</p>
        </div>
        <div>
        ...
        {family.notes && (
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 whitespace-pre-wrap">{family.notes}</p>
          </div>
        )}
      </CardContent>
    </Card>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-foreground">Members ({people.length})</h3>
        {userCanEdit && (
          <Button size="sm" asChild className="print:hidden">
            <Link href={`/families/${familyId}/people/new`}>Add member</Link>
          </Button>
        )}
      </div>
      ...
      <PersonMobileList people={people} />
    </div>
  )
}

function GivingYearTable({
  year,
  transactions,
  userCanEditGiving,
}: Readonly<{
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" asChild>
            <Link href={`/families/${familyId}/giving`}>View Full History →</Link>
          </Button>
        </div>
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-6">
            {years.map((year) => (
              <GivingYearTable
                key={year}
                year={year}
                transactions={byYear[year]}
                userCanEditGiving={userCanEditGiving}
              />
            ))}
          </div>
        )}
...
export default async function FamilyDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const result = await getFamilyForDisplay(id)
  if (!result) notFound()
  const { family, displayFamily } = result
  ...
  const giving = await getFamilyGiving(family.id, userCanSeeGiving)
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <LastUpdateLine lastUpdate={lastUpdate} />
        </div>
        <FamilyActionButtons
          familyId={family.id}
          memberCount={displayFamily.people.length}
      ...
      {pendingUpdate && <PendingUpdateBanner submissionId={pendingUpdate.id} />}

      <FamilyInfoCard family={displayFamily} />

      <MembersSection familyId={family.id} people={displayFamily.people} userCanEdit={userCanEdit} />

      {userCanSeeGiving && (
        <GivingHistoryCard
          familyId={family.id}
          byYear={giving.byYear}
          years={giving.years}
          userCanEditGiving={userCanEditGiving}
        />
      )}
