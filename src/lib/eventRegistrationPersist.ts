...
// The Serializable txn body: idempotency dedupe + authoritative capacity
// re-check + create. Split out of persistRegistration to keep the retry loop
// (and that function overall) under the cognitive-complexity limit. Runs
// entirely on `tx` — the transaction boundary is unchanged, just relocated.
async function runRegistrationTxn(
  tx: Parameters<Parameters<typeof prisma.$transaction>[0]>[0],
  event: EventWithTickets,
  priced: PricedRegistration,
  payment: PaymentFields,
): Promise<{ publicToken: string; duplicate: boolean }> {
...
// Classifies a thrown runRegistrationTxn error into its RegisterResult
// failure shape, or "retry" for a serialization conflict the caller should
// re-attempt. Rethrows anything else (an unexpected/transient DB error) —
// mirrors the original inline catch's fallthrough `throw e`.
function classifyPersistTxnError(e: unknown): { ok: false; status: number; error: string } | "retry" {
...
// Runs runRegistrationTxn with the Serializable-conflict retry loop, mapping
// its thrown sentinel errors to the RegisterResult failure shape. Split out
// of persistRegistration to keep that function's cognitive complexity down.
async function persistRegistrationRow(
  event: EventWithTickets,
  priced: PricedRegistration,
  payment: PaymentFields,
): Promise<{ ok: true; publicToken: string; duplicate: boolean } | { ok: false; status: number; error: string }> {
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      const result = await prisma.$transaction(
        (tx) => runRegistrationTxn(tx, event, priced, payment),
        { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }
      )
      return { ok: true, publicToken: result.publicToken, duplicate: result.duplicate }
    } catch (e) {
      const mapped = classifyPersistTxnError(e)
      if (mapped !== "retry") return mapped
...
// Confirmation email — fire-and-forget. A send failure is logged and never
// blocks or fails the registration that already committed. Split out of
// persistRegistration to keep that function's cognitive complexity down.
async function sendRegistrationConfirmation(
  ...
  const { firstName, lastName, email, items, totalAmount } = priced
  try {
...
export async function persistRegistration(
  ...
  const attempt = await persistRegistrationRow(event, priced, payment)
  if (!attempt.ok) return attempt
  const { publicToken, duplicate: isDuplicate } = attempt
  ...
  if (rateLimit(`regmail:${priced.emailHash}`, 5, 3_600_000)) {
    await sendRegistrationConfirmation(event, priced, payment, publicToken)
  }
